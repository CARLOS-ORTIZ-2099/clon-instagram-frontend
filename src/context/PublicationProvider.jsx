...
export const PublicationProvider = ({ children }) => {
  ...
  /*  const deletePublicationHandler = async (id) => {
    try {
      ...
      console.log(publications);
      const publicationsUpdate = publications.filter(
        (publication) => publication._id !== publicationDeleted._id
      );
  ...
  /*   const editPublicationHandler = async (id, fields) => {
    ...
    try {
      ...
      setPublication({
        ...publication,
        content: fields.content || publication.content,
        file: data?.file?.filename
          ? `/uploads/${data?.file?.filename}`
          : publication.file,
      });
      const modifyPublication = publications.map((p) =>
