...
/*   const deleteCommentHandler = async (id) => {
    try {
      ...
      const publicationUpdate = publications.map((pb) =>
        pb._id === publication._id
          ? {
              ...pb,
              comments: pb.comments.filter((comment) => comment._id !== id),
            }
          : pb
      );
...
/*   const createCommentHandler = async (id, fields) => {
    ...
    try {
      ...
      const publicationUpdate = publications.map((pb) =>
        pb._id === id ? { ...pb, comments: [...pb.comments, comment] } : pb
      );
      setPublications(publicationUpdate);
    } catch (error) {
      console.log(error);
  ...
  const editCommentHandler = async (id, fields) => {
    try {
      ...
      const editComments = publication.comments.map((comment) =>
        comment._id === id ? { ...comment, content: fields.content } : comment
      );
